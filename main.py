...
async def post_webhook(session: aiohttp.ClientSession, event: KillEvent, map_name: Optional[str], allies_faction: str, axis_faction: str) -> None:
    ...
    description = (
        ...
        f"{kill_icon} {event.weapon} • {kill_icon} {kill_type}"
    )
    ...
    if map_name:
        embed["fields"] = [{"name": "🗺️ Map", "value": str(map_name), "inline": True}]
            ...
            try:
                ...
                while True:
                    ...
                    if not startup_complete:
                        ...
                        for event in events:
                            await post_webhook(session, event, current_map, allies_faction, axis_faction)
