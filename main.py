...
def make_event_fingerprint(event: dict) -> str:
    # IMPORTANT: timestamp is intentionally NOT included.
    # Timestamp changes every poll, so including it causes duplicate webhook posts.
    return "|".join([
...
def build_embed(event: dict) -> dict:
    ...
    return {
        ...
        "color": color,
        "footer": {"text": f"{SERVER_NAME} • {get_current_utc_timestamp()}"},
    }
...
async def run() -> None:
    ...
    logger.info("Dedupe enabled: ttl=%ss cache_size=%s timestamp_in_dedupe=false", DEDUP_TTL_SECONDS, DEDUP_CACHE_SIZE)
