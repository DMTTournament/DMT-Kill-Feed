...
DEDUP_TTL_SECONDS = int(os.getenv("DEDUP_TTL_SECONDS", "300") or "300")
...
HLL_TIMESTAMP_RE = re.compile(r"^\[(?P<timestamp>[^\]]+)\]")

def extract_hll_timestamp(line: str) -> str:
    """
    Extract timestamp directly from HLL admin log line and format it as:
    06 MAY 26 - 14:34 UTC
    """
    if not line:
        return "Unknown Time"

    match = HLL_TIMESTAMP_RE.search(line)
    if not match:
        return "Unknown Time"
...
def parse_entry_object(entry) -> Optional[dict]:
    ...
    if killer and victim and weapon and killer_team and victim_team:
        ...
        return {
            ...
            "hll_timestamp": extract_hll_timestamp(message),
        }
    return None
    ...
    for pattern in [TEAM_IN_PARENS_RE, SIMPLE_KILL_RE]:
        ...
        if match:
            ...
            return {
                ...
                "hll_timestamp": extract_hll_timestamp(line),
            }
    logger.debug("Unparsed kill-like log line: %s", line)
    return None

def make_event_fingerprint(event: dict) -> str:
    return "|".join([
        normalize_text(event.get("killer", "")),
        normalize_text(event.get("killer_team", "")),
        normalize_text(event.get("victim", "")),
        normalize_text(event.get("victim_team", "")),
        normalize_text(event.get("weapon", "")),
        normalize_text(event.get("hll_timestamp", "")),
        "tk" if event.get("is_teamkill") else "kill",
    ])

def prune_seen(now: float) -> None:
    expired = [key for key, ts in seen_times.items() if now - ts > DEDUP_TTL_SECONDS]
    for key in expired:
        seen_times.pop(key, None)
...
def remember_seen(key: str) -> bool:
    ...
    if key in seen_set:
        logger.debug("Duplicate kill suppressed: %s", key)
        return False

    if len(seen_cache) == seen_cache.maxlen:
        old = seen_cache.popleft()
        seen_set.discard(old)
        seen_times.pop(old, None)
...
def build_embed(event: dict) -> dict:
    ...
    timestamp_text = event.get("hll_timestamp", "Unknown Time")

    return {
        "title": title,
        "description": description,
        "color": color,
        "footer": {"text": f"{SERVER_NAME} • {timestamp_text}"},
    }
...
async def run() -> None:
    ...
    logger.info("Dedupe enabled: ttl=%ss cache_size=%s", DEDUP_TTL_SECONDS, DEDUP_CACHE_SIZE)
        ...
        try:
            ...
            while True:
                ...
                for entry in entries:
                    ...
                    if not event.get("hll_timestamp"):
                        event["hll_timestamp"] = extract_hll_timestamp(line)

                    fingerprint = make_event_fingerprint(event)
                    if not remember_seen(fingerprint):
                        continue

                    logger.info("Parsed kill event: killer=%s victim=%s weapon=%s time=%s type=%s",
                                event["killer"], event["victim"], event["weapon"],
                                event.get("hll_timestamp", "Unknown Time"),
