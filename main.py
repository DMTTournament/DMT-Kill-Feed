...
def infer_allies_faction_from_map(map_name: str) -> str:
    ...
    if any(x in m for x in british_maps):
        return "BRITISH"

    if any(x in m for x in soviet_maps):
        return "SOVIET"
...
# KILL: Killer(Allies) -> Victim(Axis) with M1 Garand
# TEAM KILL: Killer(Allies) -> Victim(Allies) with Thompson
# KILL: Killer (Allies) -> Victim (Axis) with M1 Garand
KILL_RE = re.compile(
...
async def fetch_admin_log_lines(rcon: Rcon) -> list[str]:
    ...
    # hllrcon 1.x returns GetAdminLogResponse(entries=[...])
    if hasattr(response, "entries"):
        return [entry.message for entry in response.entries]

    # Defensive fallback for older or unexpected versions
