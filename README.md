...
## Optional test kill after deploy
Set `SEND_TEST_KILL=true` in Railway to send one sample kill embed when the service starts. To prevent spam, keep it `false` for normal use. The app sends the test only once per running Railway process, but Railway restarts will send another test if this remains `true`. Recommended: turn it on for one deploy, verify Discord, then set it back to `false`.
