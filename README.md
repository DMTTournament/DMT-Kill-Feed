...
The app connects to your HLL server RCON using `hllrcon`, polls RCON V2 `GetAdminLog`, parses kill lines, deduplicates them, and sends embeds to Discord through your webhook.

HLL kill events are read from the RCON V2 admin log command (`GetAdminLog`) through `hllrcon`. The app filters for `KILL` events, normalizes the returned admin-log entries into parser-friendly lines, deduplicates them, and posts Discord embeds. The `hllrcon` library is an async Python implementation of the HLL RCON protocol.
...
The app logs startup, the detected `hllrcon` methods, RCON connection attempts, parsed kill counts, webhook failures, and parsing errors to stdout/stderr. Railway will show these under the service logs.
